...
exports.run = async (client, message, args) => {
   if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Bu Komutu Kullanabilmek İçin **YÖNETİCİ** Yetkisine Sahip Olman Gerek.")
    ...
    const cekilisid = db.generate({ length: 6, readable: true, charset: 'alphabetic', capitalization: 'uppercase' })
    
    message.delete()
    let embed = new Discord.MessageEmbed()
    .setAuthor('🎉 Çekiliş 🎉')
    .setTitle('**' + verilecek + '**')
    .setDescription(`Aşağıdaki 🎉 Emojisine
                     Tıklayarak çekilişe katılabilirsiniz!\n
                     **Kalan süre:** ${sure}`)
